...
def save_journal_entries():
    if db is not None and st.session_state.journal_entries:
        try:
...
# Chat view
    ...
    if not st.session_state.chat_history and st.session_state.current_chat_id and db:
        try:
            chat_data = db.chats.find_one({"chat_id": st.session_state.current_chat_id})
            if chat_data:
...
# Analytics view
    ...
    # Tags analysis - Now using MongoDB data
    if st.session_state.journal_entries:
    ...
    # Chat engagement metrics - Now using MongoDB data
    if db is not None:
        try:
            ...
            if chat_count > 0:
                ...
                total_messages = sum(len(chat['messages']) for chat in db.chats.find({}))
    ...
    # Export data functionality - Updated for MongoDB
    st.markdown("---")
